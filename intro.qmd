...
Term statistics was derived from the Neo-Latin word `statisticum collegium` meaning "council of state" and the Italian word `statista` meaning "statesman" or "politician". A German word `Statistik`, got the meaning "collection and classification of data" generally in the early 19th century. This word was first introduced by Gottfried Achenwall (1749). `Statistik` was originally designated as a term for analysis of data about the state (data used by government or other administrative bodies). The term `Statistik` was introduced into English in 1791 by Sir John Sinclair when he published the first of 21 volumes titled "Statistical Account of Scotland" [@ball]. The first book to have 'Statistics' in its title was "Contributions to Vital Statistics" (1845) by Francis GP Neison, actuary[^intro-1] to the Medical Invalid and General Life Office.

[^intro-1]: actuary: A person who compiles and analyses statistics and uses them to calculate insurance risks and premiums.

![Statistical Account of Scotland by Sir John Sinclair (1791)](images/history1.png){fig-alt="Statistical Account of Scotland by Sir John Sinclair (1791)" fig-align="center" fig-height="4"}

## Statistics and Mathematics

Mathematics is like a well-organized library where everything follows strict rules and paths—once a theorem is proven, it stays true with little room for change. But statistics is more like a vast, open field where you have to adapt to new situations. Why? Because it deals with real-world data, which is often messy, unpredictable, and full of noise (also known as the error term). In statistics, you're not just proving theorems—you’re finding ways to understand data, account for uncertainties, and make informed decisions based on that data.

In biological phenomena, where we often study complex systems like plants, animals, or ecosystems, exact results are not always achievable. There are many factors influencing the outcomes that we cannot always measure or control. This is where the error term comes in. It represents the variation between observed and predicted values in a model. Even though we can try to predict patterns in biology, the results will always have some level of uncertainty or error due to the complexity and variability of living systems.

Statisticians acknowledge this uncertainty, using mathematical models to represent biological phenomena while accounting for these errors. Rather than focusing on achieving perfect results, the goal is to develop models that provide useful insights despite the inherent unpredictability. Statisticians use these models to understand and explain biological phenomena, accepting that exact predictions are often out of reach, but valuable insights can still be drawn from the data.
...
**Population Parameter** A *population parameter* is a numerical characteristic or value that describes an aspect of an entire population. It is a fixed (cons), often unknown value that represents the true measurement of a specific attribute for every member of the population. Common population parameters include the population mean, population variance, and population proportion. Since it is usually impractical or impossible to measure the entire population, parameters are often estimated using sample data.
...
A *constant* refers to a value that does not change under any circumstances. Unlike variables, constants retain the same value throughout the study. Examples of constants include mathematical values such as pi (𝝅), which is the ratio of the circumference of a circle to its diameter (𝝅 = 3.14159...), and *e*, the base of the natural logarithms (*e* = 2.71828).
...
Variables can be classified into four distinct levels of measurement, each representing a different way of organizing and interpreting data. These four levels are **nominal**, **ordinal**, **interval**, and **ratio**.
...
![Classification of variables](images/image2.jpeg){fig-alt="Classification of variables" fig-align="center" out-width="100%"}
...
Data is gathered by contacting individuals via telephone. This method is fast and allows for accurate information to be collected efficiently, making it suitable for studies that require a broad reach but still need reliable data.
...
The table below shows the number of fruits per branch in a mango tree selected from a particular plot. The data, presented in this form in which it was collected, is called *raw data*.

![Raw data set of no. of fruits per branch in a mango tree](images/freq1.png){fig-alt="Raw data set of no. of fruits per branch in a mango tree" fig-align="center" out-width="100%"}
...
![Frequency distribution table](images/fig1.4.png){fig-alt="Frequency distribution table" fig-align="center" out-width="100%"}
...
![Plant height of 20 paddy varieties](images/fig1.5.png){fig-alt="Plant height of 20 paddy varieties" fig-align="center" out-width="100%"}
...
![Grouped Frequency distribution table](images/fig1.6.png){fig-alt="Grouped Frequency distribution table" fig-align="center" out-width="100%"}
...
![lass boundary and class limits](images/fig1.7.png){fig-alt="Class boundary and class limits" fig-align="center" out-width="100%"}

[Note:]{.underline}\
Notice that the lower class boundary of the i^th^ class interval is the mean of the lower class limit of the class interval and the upper class limit of the (i-1)^th^ class interval (i = 2, 3, 4, ...). For example, in the table above the lower class boundaries of the second and the fourth class intervals are (110 + 119) /2 = 114.5 and (130 + 139)/2 = 134.5 respectively.\
It can also be seen that the upper class boundary of the i^th^ class interval is the mean of the upper class limit of the class interval and the lower class limit of the (i+1)^th^ class interval (i = 1, 2, 3, ...). Thus, in the above table the upper class boundary of the fourth class interval is (130 + 139)/2 = 134.5.
...
9,16,13,7,8,4,18,10,17,18,9,\
12,5,9,9,16,1,8,17,1,10,5,9,11,\
15,6,14,9,1,12,5,16,4,16,8,15,14,17
...
::: {.callout-tip title="The Secret Ingredient: Data!"}
