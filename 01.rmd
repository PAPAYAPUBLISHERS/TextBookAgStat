...
In this chapter we will have the introduction, definition of statistics,
collection and classification of data, formation of frequency
distribution.[@goon] [@gupta]
...
actuary[^1] to the Medical Invalid and General Life Office.

[^1]: actuary: A person who compiles and analyses statistics and uses
...
circle to its diameter (𝝅 = 3.14159...) and *e*, the base of the natural
or (Napierian) logarithms (*e*=2.71828).
...
Table shows the number of fruits per branch in a mango tree selected
from a particular plot. The data, presented in this form in which it was
...
It can be seen that, the minimum and the maximum numbers of fruits per
branch are 0 and 5, respectively. Apart from these numbers, it is
...
easily be seen that, most of the branches selected have four fruits
because number of branches having 4 fruits is 7. This information cannot
easily be obtained from the raw data. The above table is called a
...
and 144 cm, respectively. A frequency distribution giving every plant
height between 107 cm and 144 cm would be very long and would not be
very informative. The problem is to overcome by grouping the data into
classes.\
...
difference between the lower class limits of the third and the fourth
class intervals.
